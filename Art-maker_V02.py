...
# # notes for myself
# conda create --name artmaker python=3.11

# conda activate artmaker

# pip install opencv-python numpy Pillow pyinstaller


# python -O -m PyInstaller --onefile D:\art_maker\Art-maker_V02.py
...
def gorseli_renk_azalt(gorsel_yolu, parca_sayisi):
    """
    Verilen yoldaki görseli yükler ve K-Means kullanarak renk sayısını azaltır.
    Geriye işlenmiş görselin NumPy dizisini ve kullanılan renk merkezlerini döndürür.
    Önizleme amacıyla orijinal boyutta yüklenir.
    """
    try:
        ...
        # Görüntü boyutlandırma burada yapılmayacak, orijinal boyut kalsın
        # K-Means Kümeleme için veriyi hazırla
...
def palet_olustur(centers, renk_numara_eslesmesi):
    """
    Renk merkezlerini (centers) kullanarak numaralandırılmış görsel bir palet oluşturur.
    """
    KARE_BOYUT = 100
    ARA_BOŞLUK = 20
    FONT_SCALE = 1.0
...
class PaintByNumberApp:
    def __init__(self, master):
        ...
        self.zoom_factor = 1.0 # Başlangıç zoom seviyesi
    ...
    def fotografi_sec(self):
        ...
        if yol:
            ...
            self.zoom_factor = 1.0 # Yeni fotoğrafta zoom seviyesini sıfırla
    ...
    def onizleme_goster(self):
        ...
        # Görseli RGB'ye çevir ve PIL Image objesini sakla (zoom için gerekli)
        self.orijinal_pil_img = Image.fromarray(cv2.cvtColor(islenmis_img_bgr, cv2.COLOR_BGR2RGB))
        self.centers = centers # centers'ı kaydet
        self.islenmis_img_bgr = islenmis_img_bgr # BGR görüntüyü kaydet
        ...
        # --- ZOOM VE SCROLLBAR DÜZENLEMESİ BAŞLANGICI ---
        ...
        # Zoom için tekerlek olayını bağla
        self.canvas.bind("<MouseWheel>", self.on_mousewheel) # Windows/Mac
        self.canvas.bind("<Button-4>", self.on_mousewheel) # Linux (scroll up)
        self.canvas.bind("<Button-5>", self.on_mousewheel) # Linux (scroll down)

        # Görseli Canvas'ta göster
        self.update_image_on_canvas() 
        
        # --- ZOOM VE SCROLLBAR DÜZENLEMESİ SONU ---
        
        # Alt Butonlar ve Açıklama
    ...
    def on_mousewheel(self, event):
        """Fare tekerleği olayını işler ve zoom faktörünü değiştirir."""
        if event.num == 4 or event.delta > 0: # Zoom in
            self.zoom_factor *= 1.1
        elif event.num == 5 or event.delta < 0: # Zoom out
            self.zoom_factor /= 1.1
        
        # Minimum zoom seviyesi belirle
    ...
    def update_image_on_canvas(self):
        """Güncel zoom faktörüne göre görseli yeniden boyutlandırır ve canvas'ı günceller."""
        ...
        # Canvas'ın kaydırma alanını görselin boyutuna ayarla
    ...
    def yeni_renk_sayisi_gir(self):
        ...
        if yeni_sayi is not None:
            ...
            self.zoom_factor = 1.0 # Yeni işleme için zoomu sıfırla
            self.onizleme_goster() 

    def kaydetme_islemini_baslat(self, quantized_img, centers, parca_sayisi):
        """Onaylandıktan sonra tüm nihai çıktıları kaydeder."""
